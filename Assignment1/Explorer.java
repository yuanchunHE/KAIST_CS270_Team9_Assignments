...
public class Explorer extends Thread {
	...
	/* Constructer */
	public Explorer(MoveAPI _api, DataFlow df) {
		api = _api;
		dataFlow = df;
	}
	...
	private void moveDirect(int x, int y) {
		...
		while (true) {// (dp[x][y][0] + dp[x][y][1] + dp[x][y][2] + dp[x][y][3]
						// >= INF * 4) {
	...
	private void dfs(int x, int y) {
		...
		while (true) {
			if (dataFlow.isBoxDetect1() && dataFlow.isBoxDetect2()
					&& dataFlow.isRedDetect1() && dataFlow.isRedDetect2())
				break;
	...
	public void run() {
		// initialization
		posX = dataFlow.getRobotPos().x;
		posY = dataFlow.getRobotPos().y;
		posDir = dataFlow.getRobotPos().facingDirection;
